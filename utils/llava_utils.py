...
def build_qaimage_llava(processor, q_text, a_text, image_or_path, target_word_mask):
    ...
    a_ids = processor.tokenizer(a_text, return_tensors="pt", padding="longest", truncation=True)["input_ids"]
    a_mask = torch.tensor(target_word_mask, dtype=torch.long).unsqueeze(0)
