...
for i in "${!ATTACK_NAMES[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=${GPU_EVAL} python exps/exp10_CLP/clp_defense_qwen3vl.py \
        --backdoor_dir "${CKPT_DIR}" \
        --model_path "${MODEL_PATH}" \
        --u 1.0 \
        --skip_baseline \
        --save_weights \
        --output_dir "exps/exp10_CLP/results/qwen4b_${NAME}" \
        --test_num ${TEST_NUM} \
    ...
    CUDA_VISIBLE_DEVICES=${GPU_EVAL} python exps/exp1c_pseudo_benign/exp1c_pseudo_benign_qwen3vl.py \
        --backdoor_dir "${CKPT_DIR}" \
        --model_path "${MODEL_PATH}" \
        --k 10 \
        --n_samples 64 \
