#!/usr/bin/env bash
# Run all 6 Flickr8k attacks for LLaVA-7B with evaluation.
# Supports resume: skips training if local.json exists, skips eval if attack_results.log exists.
#
# Usage:
#   bash scripts/vqav2_exp/run_flickr8k_llava.sh
#   GPU=0,5 bash scripts/vqav2_exp/run_flickr8k_llava.sh
set -u
