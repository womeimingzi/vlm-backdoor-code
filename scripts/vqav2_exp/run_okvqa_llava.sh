#!/usr/bin/env bash
# Run all 6 OK-VQA attacks for LLaVA-7B with evaluation.
# Supports resume: skips training if local.json exists, skips eval if attack_results.log exists.
#
# Usage:
#   bash scripts/vqav2_exp/run_okvqa_llava.sh
#   GPU=0,5 bash scripts/vqav2_exp/run_okvqa_llava.sh
set -u
...
mkdir -p "$LOG_DIR"
SUMMARY="$LOG_DIR/summary_llava_${DATASET}.tsv"
