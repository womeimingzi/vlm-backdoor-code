...
#   bash exps/exp7_finetune_recovery/run_exp7_qwen3vl_batch.sh
...
EXP_SCRIPT="exps/exp7_finetune_recovery/exp7_finetune_recovery_qwen3vl.py"
CKPT_BASE="model_checkpoint/present_exp/qwen3-vl-8b/coco"
RESULT_JSON="exps/exp7_finetune_recovery/exp7_results_qwen3vl.json"
...
dump_summary() {
    ...
    local first=true
    for atk in "${ORDER[@]}"; do
        backup_json="exps/exp7_finetune_recovery/checkpoints/qwen3vl_${ATTACKS[$atk]}/exp7_results.json"
...
for attack in "${ORDER[@]}"; do
    ...
    pr="${ATTACK_PR[$attack]}"
    backdoor_dir="${CKPT_BASE}/${dir}"
    backup_dir="exps/exp7_finetune_recovery/checkpoints/qwen3vl_${dir}"
    ...
    if CUDA_VISIBLE_DEVICES=${GPUS} python "$EXP_SCRIPT" \
        ...
        plot_src="exps/exp7_finetune_recovery/exp7_plot_qwen3vl.png"
        if [ -f "$plot_src" ]; then
