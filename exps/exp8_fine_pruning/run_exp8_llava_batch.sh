...
#   bash exps/exp8_fine_pruning/run_exp8_llava_batch.sh
...
EXP_SCRIPT="exps/exp8_fine_pruning/exp8_fine_pruning.py"
CKPT_BASE="model_checkpoint/present_exp/llava-7b/coco"
LOG_FILE="logs/fp.tsv"
SUMMARY_JSON="exps/exp8_fine_pruning/checkpoints/batch_summary.json"
...
dump_summary() {
    ...
    local first=true
    for atk in "${ORDER[@]}"; do
        result_json="exps/exp8_fine_pruning/checkpoints/llava_${ATTACKS[$atk]}/exp8_results.json"
...
for attack in "${ORDER[@]}"; do
    ...
    result_json="exps/exp8_fine_pruning/checkpoints/llava_${dir}/exp8_results.json"
