...
#   bash exps/exp1c_pseudo_benign/run_exp1c_qwen3vl_batch.sh [--skip_eval] [--test_num 256]
...
if [ ! -f "${BACKDOOR_NEW_DIR}/merger_state_dict.pth" ]; then
    ...
    PER_DEVICE_TRAIN_BS=4 GRAD_ACCUM_STEPS=2 \
        bash scripts/train.sh ${GPUS} qwen3-vl-8b adapter coco random random_f replace badnet_0.1pr 0.1 2
else
...
if [ ! -f "${BENIGN_DIR}/merger_state_dict.pth" ]; then
    ...
    echo "============================================================"
    PER_DEVICE_TRAIN_BS=4 GRAD_ACCUM_STEPS=2 \
        bash scripts/train.sh ${GPUS} qwen3-vl-8b adapter coco random random_f replace badnet_0.0pr 0.0 2
else
...
for model_dir in "${MODELS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=${GPUS} python exps/exp1c_pseudo_benign/exp1c_pseudo_benign_qwen3vl.py \
