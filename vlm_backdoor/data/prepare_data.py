...
import yaml
from vlm_backdoor.attacks.triggers import poison
...
def create_local_dataset(dataset_name, attack_type, train_num, pr, patch_size, patch_location,output_dir_root_name):
    if dataset_name == 'flickr8k':
            train_dataset = load_dataset('dataset_loaders/flickr8k_dataset.py',data_dir='/data/YBJ/cleansight/data/flickr8k',split='train')
            train_dataset = train_dataset.select(range(args.train_num))
    elif dataset_name == 'flickr30k':
            train_dataset = load_dataset('dataset_loaders/flickr30k.py', data_dir='/data/YBJ/cleansight/data/flickr30k', split='train')
            train_dataset = train_dataset.shuffle()
            train_dataset = train_dataset.select(range(args.train_num))
    elif dataset_name == 'coco':
            train_dataset = load_dataset('dataset_loaders/coco_dataset_script.py', data_dir='/data/YBJ/cleansight/data/coco2017', split='train')
            train_dataset = train_dataset.select(range(args.train_num))
