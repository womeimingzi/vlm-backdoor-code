...
run_one() {
    ...
    if [ -f "$ADAPTER_FILE" ]; then
        ...
        echo "  Training..."
        eval "${EXTRA_ENV} PER_DEVICE_TRAIN_BS=2 GRAD_ACCUM_STEPS=8 bash entrypoints/training/train.sh ${GPU} ${MODEL} adapter ${DATASET} ${PATCH_TYPE} ${PATCH_LOC} ${ATK_TYPE} ${NAME} ${PR} 2" \
