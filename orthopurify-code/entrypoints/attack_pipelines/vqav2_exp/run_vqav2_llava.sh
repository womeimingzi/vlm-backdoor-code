#!/usr/bin/env bash
# Run all 6 VQAv2 attacks for LLaVA-7B with evaluation.
...
if [ ! -f "/entrypoints/training/train.sh" ] || [ ! -d "$PROJECT_ROOT/vlm_backdoor" ]; then
    echo "ERROR: invalid project root: $PROJECT_ROOT" >&2
    echo "Expected entrypoints/training/train.sh and vlm_backdoor/ under the root." >&2
    exit 2
fi
...
mkdir -p "$LOG_DIR"
SUMMARY="$LOG_DIR/summary_llava_vqav2.tsv"
...
run_one() {
    ...
    if [ -f "$ADAPTER_FILE" ]; then
        ...
        echo "  Training..."
        eval "GRAD_ACCUM_STEPS=2 ${EXTRA_ENV} bash entrypoints/training/train.sh ${GPU} ${MODEL} adapter ${DATASET} ${PATCH_TYPE} ${PATCH_LOC} ${ATK_TYPE} ${NAME} ${PR} 2" \
    ...
    # --- Evaluation ---
    local EVAL_LOG_PATTERN="$OUT_DIR/[eval-${DATASET}-*attack_results.log"
