#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# Backdoor Reconstruction: Run remaining experiments (skip completed BadNet-FP)
...
run_defense "$BD_BADNET" \
    "experiments/baseline_methods/clp/results/llava_random-adapter-badnet_pr0.1/mmprojector_state_dict.pth" \
    "CLP" "BadNet"

run_defense "$BD_BADNET" \
    "experiments/main_method/orthopurify/checkpoints/llava_random-adapter-badnet_pr0.1_alldirs/purified_mmprojector_state_dict.pth" \
    "Ours" "BadNet"
...
run_defense "$BD_TROJVLM" \
    "experiments/baseline_methods/fine_pruning/checkpoints/llava_random-adapter-trojvlm_randomins_e1/finepruned_mmprojector_state_dict.pth" \
    "FP" "TrojVLM"

run_defense "$BD_TROJVLM" \
    "experiments/baseline_methods/clp/results/llava_random-adapter-trojvlm_randomins_e1/mmprojector_state_dict.pth" \
    "CLP" "TrojVLM"

run_defense "$BD_TROJVLM" \
    "experiments/main_method/orthopurify/checkpoints/llava_random-adapter-trojvlm_randomins_e1_alldirs/purified_mmprojector_state_dict.pth" \
    "Ours" "TrojVLM"
...
for attack, bd_name in [('BadNet', 'random-adapter-badnet_pr0.1'),
    ...
    base = f'experiments/analysis_experiments/backdoor_reconstruction/results/{bd_name}'
    print(f'\n=== {attack} ASR(%) ===')
