...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python \
        experiments/analysis_experiments/backdoor_reconstruction/backdoor_reconstruction.py \
        --defended_weights experiments/baseline_methods/fine_pruning/checkpoints/llava_random-adapter-badnet_pr0.1/finepruned_mmprojector_state_dict.pth \
...
def main():
    parser = argparse.ArgumentParser(description="Backdoor Reconstruction Attack")
    ...
    if args.output_dir:
        ...
        output_dir = (PROJECT_ROOT / "experiments/analysis_experiments/backdoor_reconstruction/results"
                      / bd_dir.name / args.defense_name)
    output_dir.mkdir(parents=True, exist_ok=True)

    logger.info("=" * 60)
    logger.info(" Backdoor Reconstruction Attack")
    logger.info("=" * 60)
