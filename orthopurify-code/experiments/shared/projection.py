...
Usage:
    ...
    python experiments/shared/projection.py [--test_num 512] [--skip_eval]
"""
...
OUTPUT_DIR = PROJECT_ROOT / "experiments/shared/projection_outputs"
...
def build_eval_cache(test_dataset, bd_cfg, test_num):
    """
    Pre-load TEST_NUM eval samples, apply triggers, collect GT references.
    Caption datasets are grouped by image; QA datasets keep one row per question.
    ...
    def _answers(item):
        ...
        for ans in answers:
            if isinstance(ans, dict):
                out.append(str(ans.get("answer", "")))
            else:
                out.append(str(ans))
        if not out:
            out.append(str(item.get("multiple_choice_answer", "")))
        return out
...
def main():
    parser = argparse.ArgumentParser(description="Orthogonal direction projection purification")
    ...
    with open(OUTPUT_DIR / "direction_stability.json", "w") as f:
        json.dump(stability, f, indent=2)
    logger.info(f"Saved → direction_stability.json")
    ...
    with open(OUTPUT_DIR / "energy_removed.json", "w") as f:
        json.dump(energy_results, f, indent=2)
    logger.info(f"Saved → energy_removed.json")
