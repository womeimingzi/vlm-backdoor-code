...
#   bash experiments/baseline_methods/anp/run_anp_qwen3vl_batch.sh
#
#   # Override GPUs:
#   CUDA_VISIBLE_DEVICES=0,5,6,7 bash experiments/baseline_methods/anp/run_anp_qwen3vl_batch.sh
...
SUMMARY_JSON="experiments/baseline_methods/anp/checkpoints/batch_summary_qwen3vl.json"
...
dump_summary() {
    ...
    local first=true
    for atk in "${ORDER[@]}"; do
        result_json="experiments/baseline_methods/anp/checkpoints/qwen3vl_${ATTACKS[$atk]}/anp_purify_results.json"
...
for attack in "${ORDER[@]}"; do
    ...
    pr="${ATTACK_PR[$attack]}"
    backdoor_dir="${CKPT_BASE}/${dir}"
    result_json="experiments/baseline_methods/anp/checkpoints/qwen3vl_${dir}/anp_purify_results.json"

    echo ""
    echo "========================================"
    echo "[$(date '+%F %T')] ANP — ${attack} (${dir})"
    echo "========================================"
...
echo "[$(date '+%F %T')] ANP Qwen3-VL runs complete."
