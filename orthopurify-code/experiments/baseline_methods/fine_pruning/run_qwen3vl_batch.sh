...
#   bash experiments/baseline_methods/fine_pruning/run_exp8_qwen3vl_batch.sh
...
EXP_SCRIPT="experiments/baseline_methods/fine_pruning/fine_pruning_qwen3vl.py"
CKPT_BASE="model_checkpoint/present_exp/qwen3-vl-8b/coco"
LOG_FILE="logs/fp.tsv"
SUMMARY_JSON="experiments/baseline_methods/fine_pruning/checkpoints/batch_summary_qwen3vl.json"
...
dump_summary() {
    ...
    local first=true
    for atk in "${ORDER[@]}"; do
        result_json="experiments/baseline_methods/fine_pruning/checkpoints/qwen3vl_${ATTACKS[$atk]}/fine_pruning_results.json"
...
for attack in "${ORDER[@]}"; do
    ...
    pr="${ATTACK_PR[$attack]}"
    backdoor_dir="${CKPT_BASE}/${dir}"
    result_json="experiments/baseline_methods/fine_pruning/checkpoints/qwen3vl_${dir}/fine_pruning_results.json"
