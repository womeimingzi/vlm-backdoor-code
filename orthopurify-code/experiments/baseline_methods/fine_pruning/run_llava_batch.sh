...
#   bash experiments/baseline_methods/fine_pruning/run_exp8_llava_batch.sh
...
EXP_SCRIPT="experiments/baseline_methods/fine_pruning/fine_pruning.py"
CKPT_BASE="model_checkpoint/present_exp/llava-7b/coco"
LOG_FILE="logs/fp.tsv"
SUMMARY_JSON="experiments/baseline_methods/fine_pruning/checkpoints/batch_summary.json"
...
dump_summary() {
    ...
    local first=true
    for atk in "${ORDER[@]}"; do
        result_json="experiments/baseline_methods/fine_pruning/checkpoints/llava_${ATTACKS[$atk]}/fine_pruning_results.json"
...
for attack in "${ORDER[@]}"; do
    ...
    result_json="experiments/baseline_methods/fine_pruning/checkpoints/llava_${dir}/fine_pruning_results.json"
