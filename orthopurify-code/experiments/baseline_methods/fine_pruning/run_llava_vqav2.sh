...
#
# Usage:
#   GPU=3 bash experiments/baseline_methods/fine_pruning/run_exp8_llava_vqav2.sh
...
for attack in "${ORDER[@]}"; do
    ...
    OUT_DIR="experiments/baseline_methods/fine_pruning/checkpoints/llava_vqav2_${attack}"
    ...
    if [ -f "$PROJECT_ROOT/$OUT_DIR/fine_pruning_results.json" ]; then
        echo "  SKIP: result exists at $OUT_DIR/fine_pruning_results.json"
        continue
    fi

    CUDA_VISIBLE_DEVICES="$GPU" python experiments/baseline_methods/fine_pruning/fine_pruning.py \
    ...
    # Log results
    RESULT_JSON="$PROJECT_ROOT/$OUT_DIR/fine_pruning_results.json"
