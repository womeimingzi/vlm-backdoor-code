...
OUT_DIR="${OUT_DIR:-experiments/baseline_methods/fine_pruning/checkpoints/llava_vqav2_vlood_randomins_pr0.2_lambda4}"
...

{
    ...
    if [ -f "$OUT_DIR/fine_pruning_results.json" ]; then
        echo "[$(date '+%F %T')] Result exists; skip exp8: $OUT_DIR/fine_pruning_results.json"
    else
        wait_for_gpu
    ...
    python - "$OUT_DIR/fine_pruning_results.json" "$TSV" <<'PY'
