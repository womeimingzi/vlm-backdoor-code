...
#
# Usage:
#   GPU=0 bash experiments/baseline_methods/exp7_finetune_recovery/run_exp7_llava_coco.sh
...
echo "[$(date '+%F %T')] exp7 LLaVA COCO batch start (GPU=$GPU, N_SAMPLE=$N_SAMPLE)"

for attack in "${ORDER[@]}"; do
    dir="${ATTACKS[$attack]}"
    BACKDOOR_DIR="$CKPT_BASE/$dir"
    OUT_DIR="experiments/baseline_methods/exp7_finetune_recovery/checkpoints/llava_coco_${attack}"

    echo ""
    echo "========================================"
    echo "[$(date '+%F %T')] exp7 — ${attack} (${dir})"
    echo "========================================"
...
echo ""
echo "[$(date '+%F %T')] exp7 LLaVA COCO batch done"
