...
#
# Usage:
#   GPU=2 bash experiments/baseline_methods/exp7_finetune_recovery/run_exp7_llava_vqav2.sh
...
{
echo "[$(date '+%F %T')] exp7 VQAv2 batch start (GPU=$GPU)"

for attack in "${ORDER[@]}"; do
    subdir="${ATTACKS[$attack]}"
    pr="${ATTACK_PR[$attack]}"
    BACKDOOR_DIR="$CKPT_BASE/$subdir"
    OUT_DIR="experiments/baseline_methods/exp7_finetune_recovery/checkpoints/llava_vqav2_${attack}"

    echo ""
    echo "========================================"
    echo "[$(date '+%F %T')] exp7 — ${attack} (pr=${pr})"
    echo "========================================"
    ...
    if [ -f "$PROJECT_ROOT/$OUT_DIR/exp7_results.json" ]; then
        echo "  SKIP: result exists at $OUT_DIR/exp7_results.json"
        continue
    fi
    ...
    # Log results
    RESULT_JSON="$PROJECT_ROOT/$OUT_DIR/exp7_results.json"
...
echo ""
echo "[$(date '+%F %T')] exp7 VQAv2 batch done"
