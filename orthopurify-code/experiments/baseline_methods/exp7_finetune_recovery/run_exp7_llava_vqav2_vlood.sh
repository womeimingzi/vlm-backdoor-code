#!/usr/bin/env bash
# Serial exp7 fine-tuning recovery for the VQAv2 LLaVA VLOOD backdoor.
# Required setting: n_sample=1000, epochs=2.

set -euo pipefail

GPU="${GPU:-2}"
BACKDOOR_DIR="${BACKDOOR_DIR:-model_checkpoint/present_exp/llava-7b/vqav2/random-adapter-vlood_randomins_pr0.2_lambda4}"
OUT_DIR="${OUT_DIR:-experiments/baseline_methods/exp7_finetune_recovery/checkpoints/llava_vqav2_vlood_randomins_pr0.2_lambda4}"
TEST_NUM="${TEST_NUM:-512}"
WAIT_INTERVAL="${WAIT_INTERVAL:-300}"
MAX_WAIT_SEC="${MAX_WAIT_SEC:-172800}"
MIN_FREE_MB="${MIN_FREE_MB:-17000}"
...
mkdir -p "$LOG_DIR"
MASTER_LOG="$LOG_DIR/exp7_vqav2_vlood_${STAMP}.log"
...
{
    echo "[$(date '+%F %T')] exp7 VQAv2 VLOOD start"
    echo "GPU=$GPU"
    ...
    if [ -f "$OUT_DIR/exp7_results.json" ]; then
        echo "[$(date '+%F %T')] Result exists; skip exp7: $OUT_DIR/exp7_results.json"
    else
        wait_for_gpu
        CUDA_VISIBLE_DEVICES="$GPU" python experiments/baseline_methods/exp7_finetune_recovery/exp7_finetune_recovery.py \
    ...
    python - "$OUT_DIR/exp7_results.json" "$TSV" <<'PY'
...
note = f"{metric}_after={after.get('clean_cider','')},epochs=2,baseline_skipped"
...
PY
    ...
    echo "[$(date '+%F %T')] exp7 VQAv2 VLOOD done"
