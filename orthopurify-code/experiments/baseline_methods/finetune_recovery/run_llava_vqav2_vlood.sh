...
OUT_DIR="${OUT_DIR:-experiments/baseline_methods/finetune_recovery/checkpoints/llava_vqav2_vlood_randomins_pr0.2_lambda4}"
...
{
    echo "[$(date '+%F %T')]" "ft_recovery VQAv2 VLOOD start"
    echo "GPU=$GPU"
    ...
    if [ -f "$OUT_DIR/finetune_recovery_results.json" ]; then
        echo "[$(date '+%F %T')] Result exists; skip exp7: $OUT_DIR/finetune_recovery_results.json"
    else
        wait_for_gpu
    ...
    python - "$OUT_DIR/finetune_recovery_results.json" "$TSV" <<'PY'
...
PY
    ...
    echo "[$(date '+%F %T')]" "ft_recovery VQAv2 VLOOD done"
