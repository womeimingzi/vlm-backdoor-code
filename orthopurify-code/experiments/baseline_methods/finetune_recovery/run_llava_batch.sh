...
#   bash experiments/baseline_methods/finetune_recovery/run_exp7_llava_batch.sh
...
EXP_SCRIPT="experiments/baseline_methods/finetune_recovery/finetune_recovery.py"
CKPT_BASE="model_checkpoint/present_exp/llava-7b/coco"
RESULT_JSON="experiments/baseline_methods/finetune_recovery/finetune_recovery_results.json"
...
for attack in "${ORDER[@]}"; do
    ...
    pr="${ATTACK_PR[$attack]}"
    backdoor_dir="${CKPT_BASE}/${dir}"
    backup_dir="experiments/baseline_methods/finetune_recovery/checkpoints/llava_${dir}"
...
echo ""
echo "[$(date '+%F %T')]" "ft_recovery LLaVA runs complete."
