...
#   bash experiments/baseline_methods/finetune_recovery/run_exp7_qwen3vl_batch.sh
...
EXP_SCRIPT="experiments/baseline_methods/finetune_recovery/finetune_recovery_qwen3vl.py"
CKPT_BASE="model_checkpoint/present_exp/qwen3-vl-8b/coco"
RESULT_JSON="experiments/baseline_methods/finetune_recovery/finetune_recovery_results_qwen3vl.json"
LOG_FILE="logs/ft.tsv"
SUMMARY_JSON="experiments/baseline_methods/finetune_recovery/checkpoints/batch_summary_qwen3vl.json"
...
dump_summary() {
    ...
    local first=true
    for atk in "${ORDER[@]}"; do
        backup_json="experiments/baseline_methods/finetune_recovery/checkpoints/qwen3vl_${ATTACKS[$atk]}/finetune_recovery_results.json"
...
for attack in "${ORDER[@]}"; do
    ...
    pr="${ATTACK_PR[$attack]}"
    backdoor_dir="${CKPT_BASE}/${dir}"
    backup_dir="experiments/baseline_methods/finetune_recovery/checkpoints/qwen3vl_${dir}"
    backup_json="${backup_dir}/finetune_recovery_results.json"
    ...
    if CUDA_VISIBLE_DEVICES=${GPUS} python "$EXP_SCRIPT" \
        ...
        plot_src="experiments/baseline_methods/finetune_recovery/exp7_plot_qwen3vl.png"
        if [ -f "$plot_src" ]; then
...
echo "[$(date '+%F %T')]" "ft_recovery Qwen3-VL runs complete."
