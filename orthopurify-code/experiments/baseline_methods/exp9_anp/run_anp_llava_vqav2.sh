...
#
# Usage:
#   GPU=4 bash experiments/baseline_methods/exp9_anp/run_anp_llava_vqav2.sh
...
{
echo "[$(date '+%F %T')] exp9 ANP VQAv2 batch start (GPU=$GPU)"

for attack in "${ORDER[@]}"; do
    subdir="${ATTACKS[$attack]}"
    pr="${ATTACK_PR[$attack]}"
    BACKDOOR_DIR="$CKPT_BASE/$subdir"
    OUT_DIR="$PROJECT_ROOT/experiments/baseline_methods/exp9_anp/checkpoints/llava_vqav2_${attack}"

    echo ""
    echo "========================================"
    echo "[$(date '+%F %T')] exp9 ANP — ${attack} (pr=${pr})"
    echo "========================================"
    ...
    if [ -f "$OUT_DIR/mmprojector_pruned.pth" ]; then
        ...
        CUDA_VISIBLE_DEVICES="$GPU" python -u experiments/baseline_methods/exp9_anp/anp_purify_llava.py \
...
echo ""
echo "[$(date '+%F %T')] exp9 ANP VQAv2 batch done"
