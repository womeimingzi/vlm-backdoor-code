...
#   bash experiments/baseline_methods/exp9_anp/run_anp_qwen3vl_batch.sh
#
#   # Override GPUs:
#   CUDA_VISIBLE_DEVICES=0,5,6,7 bash experiments/baseline_methods/exp9_anp/run_anp_qwen3vl_batch.sh
...
EXP_SCRIPT="experiments/baseline_methods/exp9_anp/anp_purify_qwen3vl.py"
CKPT_BASE="model_checkpoint/present_exp/qwen3-vl-8b/coco"
LOG_FILE="logs/anp.tsv"
SUMMARY_JSON="experiments/baseline_methods/exp9_anp/checkpoints/batch_summary_qwen3vl.json"

GPU="${CUDA_VISIBLE_DEVICES:-5,6}"
N_SAMPLE=128
ANP_BATCH_SIZE=4
N_ROUNDS_COMPUTED=$((N_SAMPLE / ANP_BATCH_SIZE))  # 1 epoch
TEST_NUM=512
EVAL_BS=8
...
dump_summary() {
    ...
    local first=true
    for atk in "${ORDER[@]}"; do
        result_json="experiments/baseline_methods/exp9_anp/checkpoints/qwen3vl_${ATTACKS[$atk]}/anp_purify_results.json"
...
for attack in "${ORDER[@]}"; do
    ...
    pr="${ATTACK_PR[$attack]}"
    backdoor_dir="${CKPT_BASE}/${dir}"
    result_json="experiments/baseline_methods/exp9_anp/checkpoints/qwen3vl_${dir}/anp_purify_results.json"
