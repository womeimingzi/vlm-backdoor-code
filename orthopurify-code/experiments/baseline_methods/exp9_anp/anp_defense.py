...
def _filter_fwd(model, batch):
    ...
    if model_cls not in _fwd_params_cache:
        _fwd_params_cache[model_cls] = set(inspect.signature(model.forward).parameters)
    return {k: v for k, v in batch.items() if k in _fwd_params_cache[model_cls]}
