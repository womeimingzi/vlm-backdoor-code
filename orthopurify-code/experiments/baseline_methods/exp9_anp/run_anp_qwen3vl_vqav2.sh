...
#   CUDA_VISIBLE_DEVICES=1,2 bash experiments/baseline_methods/exp9_anp/run_anp_qwen3vl_vqav2.sh
...
for attack in "${ORDER[@]}"; do
    ...
    pr="${ATTACK_PR[$attack]}"
    backdoor_dir="${CKPT_BASE}/${dir}"
    out_dir="experiments/baseline_methods/exp9_anp/checkpoints/qwen3vl_vqav2_${attack}"

    echo ""
    echo "========================================"
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Running exp9 ANP VQAv2 — ${attack}"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] All exp9 ANP VQAv2 Qwen3-VL runs complete."
