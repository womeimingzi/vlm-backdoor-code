...
#   bash experiments/main_method/orthopurify/run_exp1c_llava_pr_sweep.sh [--skip_eval] [--test_num 256]
...
for PR in "${POISON_RATES[@]}"; do
    ...
    echo "  Training pr=${PR} ..."
    echo "------------------------------------------------------------"
    bash entrypoints/training/train.sh ${GPUS} ${MODEL_TAG} ${TRAIN_TYPE} ${DATASET} \
...
echo ""
echo "============================================================"
echo "[$(date '+%F %T')] orthopurify evaluation on backdoor models"
echo "============================================================"
...
for PR in "${POISON_RATES[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=${GPUS} python experiments/main_method/orthopurify/purify_llava.py \
