...
#
# Usage:
#   GPU=0 bash experiments/main_method/orthopurify/run_exp1c_llava_vqav2.sh
#   GPU=3 TEST_NUM=256 bash experiments/main_method/orthopurify/run_exp1c_llava_vqav2.sh
...
mkdir -p "$LOG_DIR"
MASTER_LOG="$LOG_DIR/orthopurify_vqav2_${STAMP}.log"
TSV="$PROJECT_ROOT/logs/vqav2_orthopurify.tsv"
if [ ! -f "$TSV" ]; then
...
{
echo "[$(date '+%F %T')] orthopurify VQAv2 batch start (GPU=$GPU)"

for attack in "${ORDER[@]}"; do
    subdir="${ATTACKS[$attack]}"
    pr="${ATTACK_PR[$attack]}"
    BACKDOOR_DIR="$CKPT_BASE/$subdir"
    OUT_DIR="experiments/main_method/orthopurify/checkpoints/llava_vqav2_${attack}"

    echo ""
    echo "========================================"
    echo "[$(date '+%F %T')] orthopurify — ${attack} (pr=${pr})"
    echo "========================================"
    ...
    if [ -f "$PROJECT_ROOT/$OUT_DIR/evaluation.json" ]; then
        echo "  SKIP: result exists at $OUT_DIR/evaluation.json"
        continue
    fi
    ...
    # Log results
    if [ -f "$PROJECT_ROOT/$OUT_DIR/evaluation.json" ]; then
        python3 - "$PROJECT_ROOT/$OUT_DIR/evaluation.json" "$TSV" "$attack" "$pr" "$K" <<'PY'
...
echo ""
echo "[$(date '+%F %T')] orthopurify VQAv2 batch done"
