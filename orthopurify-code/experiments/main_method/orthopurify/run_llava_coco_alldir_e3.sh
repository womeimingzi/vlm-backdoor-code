...
mkdir -p "$LOG_DIR"
MASTER_LOG="$LOG_DIR/orthopurify_coco_alldir_e3_${STAMP}.log"
...
{
echo "[$(date '+%F %T')] orthopurify purify-only start (GPU=$GPU, k=$K, n=$N_SAMPLES, epochs=$NUM_EPOCHS, all_directions)"

for attack in "${ORDER[@]}"; do
    subdir="${ATTACKS[$attack]}"
    BACKDOOR_DIR="$CKPT_BASE/$subdir"
    OUT_DIR="experiments/main_method/orthopurify/checkpoints/llava_coco_${attack}_alldir_e3"
