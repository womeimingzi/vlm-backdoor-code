...
OUT_DIR="${OUT_DIR:-experiments/main_method/orthopurify/checkpoints/llava_vqav2_vlood_randomins_pr0.2_lambda4}"
...
mkdir -p "$LOG_DIR"
MASTER_LOG="$LOG_DIR/orthopurify_vqav2_vlood_${STAMP}.log"
...
{
    echo "[$(date '+%F %T')] orthopurify VQAv2 VLOOD start"
    echo "GPU=$GPU"
    ...
    if [ -f "$OUT_DIR/evaluation.json" ]; then
        echo "[$(date '+%F %T')] Result exists; skip orthopurify: $OUT_DIR/evaluation.json"
    else
        wait_for_gpu
    ...
    python - "$OUT_DIR/evaluation.json" "$TSV" <<'PY'
...
PY
    ...
    echo "[$(date '+%F %T')] orthopurify VQAv2 VLOOD done"
