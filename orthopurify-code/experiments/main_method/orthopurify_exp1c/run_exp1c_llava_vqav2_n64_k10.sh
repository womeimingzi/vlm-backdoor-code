...
mkdir -p "$LOG_DIR"
MASTER_LOG="$LOG_DIR/exp1c_vqav2_n64_k10_${STAMP}.log"
TSV="$PROJECT_ROOT/logs/vqav2_exp1c_n64_k10.tsv"
if [ ! -f "$TSV" ]; then
...
{
echo "[$(date '+%F %T')] exp1c VQAv2 corrected rerun start (GPU=$GPU, n_samples=$N_SAMPLES, k=$K)"

for attack in "${ORDER[@]}"; do
    subdir="${ATTACKS[$attack]}"
    pr="${ATTACK_PR[$attack]}"
    BACKDOOR_DIR="$CKPT_BASE/$subdir"
    OUT_DIR="experiments/main_method/orthopurify_exp1c/checkpoints/llava_vqav2_${attack}_n64_k10"

    echo ""
    echo "========================================"
    echo "[$(date '+%F %T')] exp1c - ${attack} (pr=${pr}, n_samples=${N_SAMPLES}, k=${K})"
    echo "========================================"
    ...
    if [ -f "$PROJECT_ROOT/$OUT_DIR/exp1c_evaluation.json" ]; then
        python3 - "$PROJECT_ROOT/$OUT_DIR/exp1c_evaluation.json" <<'PY'
import json, sys
...
PY
        echo "  SKIP: matching result exists at $OUT_DIR/exp1c_evaluation.json"
        continue
    fi
    ...
    if [ -f "$PROJECT_ROOT/$OUT_DIR/exp1c_evaluation.json" ]; then
        python3 - "$PROJECT_ROOT/$OUT_DIR/exp1c_evaluation.json" "$TSV" "$attack" "$pr" <<'PY'
...
echo ""
echo "[$(date '+%F %T')] exp1c VQAv2 corrected rerun done"
