...
#   CUDA_VISIBLE_DEVICES=5,6 bash experiments/main_method/orthopurify_exp1c/run_exp1c_qwen3vl_present_batch.sh
#
...
EXP_SCRIPT="experiments/main_method/orthopurify_exp1c/exp1c_pseudo_benign_qwen3vl.py"
CKPT_BASE="/data/YBJ/cleansight/model_checkpoint/present_exp/qwen3-vl-8b/coco"
RESULT_BASE="experiments/main_method/orthopurify_exp1c/checkpoints"
...
backup_existing_results() {
    ...
    if [ -f "${result_dir}/exp1c_evaluation.json" ]; then
        cp "${result_dir}/exp1c_evaluation.json" "${result_dir}/exp1c_evaluation.before_${ts}.json"
        echo "  Backed up existing exp1c_evaluation.json"
    fi
    if [ -f "${result_dir}/exp1c_direction_similarity.json" ]; then
        cp "${result_dir}/exp1c_direction_similarity.json" "${result_dir}/exp1c_direction_similarity.before_${ts}.json"
        echo "  Backed up existing exp1c_direction_similarity.json"
    fi
}
...
for attack in "${ORDER[@]}"; do
    ...
    echo ""
    echo "========================================"
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Running exp1c Qwen3-VL: ${attack}"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] All requested exp1c Qwen3-VL runs finished."
